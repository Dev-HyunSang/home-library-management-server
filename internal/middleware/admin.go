...
func AdminAPIKeyMiddleware(apiKeyUseCase domain.AdminAPIKeyUseCase) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		if providedKey == "" {
			logger.Sugar().Warn("Admin API Key가 제공되지 않았습니다")
		...
		logger.Sugar().Infof("Admin API Key 인증 성공: %s (%s)", apiKey.Name, apiKey.KeyPrefix)
		return ctx.Next()
	}
}
...
func AdminBootstrapMiddleware(bootstrapKey string) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		if bootstrapKey == "" {
			logger.Sugar().Error("ADMIN_BOOTSTRAP_KEY가 설정되지 않았습니다")
		...
		if providedKey == "" {
			logger.Sugar().Warn("Admin Bootstrap Key가 제공되지 않았습니다")
			return ctx.Status(fiber.StatusUnauthorized).JSON(handler.ErrorHandler(domain.ErrInvalidToken))
		}

		if providedKey != bootstrapKey {
			logger.Sugar().Warn("유효하지 않은 Admin Bootstrap Key")
			return ctx.Status(fiber.StatusForbidden).JSON(handler.ErrorHandler(domain.ErrPermissionDenied))
		}

		logger.Sugar().Info("Admin Bootstrap Key 인증 성공")
		return ctx.Next()
	}
}
