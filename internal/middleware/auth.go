...
func JWTAuthMiddleware(authUseCase domain.AuthUseCase) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		if err != nil {
			logger.Sugar().Errorf("JWT 토큰 인증에 실패했습니다: %v", err)
		...
		if !allowed {
			logger.Sugar().Warnf("Rate limit 초과: 사용자ID %s", userID.String())
		...
		logger.Sugar().Infof("JWT 미들웨어를 통해 사용자 인증이 완료되었습니다 / 사용자ID: %s", userID.String())
...
func LoginRateLimitMiddleware(authUseCase domain.AuthUseCase) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		logger.Sugar().Infof("로그인 요청 - IP: %s", clientIP)
