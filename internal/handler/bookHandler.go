...
func (h *BookHandler) SaveBookHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.Sugar().Errorf("책을 저장하는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}

	logger.Sugar().Infof("책이 성공적으로 저장되었습니다 / 책ID: %s, 사용자ID: %s", result.ID.String(), userID.String())
...
func (h *BookHandler) UpdateBookHandler(ctx *fiber.Ctx) error {
	...
	if len(bookID) == 0 {
		logger.Sugar().Error("책 ID가 입력되지 않았습니다.")
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}

	parsedBookID, err := uuid.Parse(bookID)
	if err != nil {
		logger.Sugar().Errorf("잘못된 책 ID 형식입니다: %v", err)
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}
	...
	if err != nil {
		if ent.IsNotFound(err) {
			logger.Sugar().Errorf("해당 책을 찾을 수 없습니다: %v", err)
			return ctx.Status(fiber.StatusNotFound).JSON(ErrorHandler(domain.ErrNotFound))
		}
		logger.Sugar().Errorf("책 조회 중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}

	if existingBook.OwnerID != userID {
		logger.Sugar().Error("해당 책의 소유자가 아닙니다.")
	...
	if err := h.bookUseCase.Edit(parsedBookID, updatedBook); err != nil {
		logger.Sugar().Errorf("책을 수정하는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}

	logger.Sugar().Infof("책이 성공적으로 수정되었습니다 / 책ID: %s, 사용자ID: %s", bookID, userID.String())
...
func (h *BookHandler) GetBookHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.Sugar().Errorf("잘못된 사용자 ID 형식입니다: %v", err)
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}

	if tokenUserID != parsedUserID {
		logger.Sugar().Error("본인의 책만 조회할 수 있습니다.")
		return ctx.Status(fiber.StatusForbidden).JSON(ErrorHandler(domain.ErrPermissionDenied))
	}
	...
	logger.Sugar().Infof("책을 성공적으로 조회했습니다 / 책ID: %s, 사용자ID: %s", bookID, userID)
...
func (h *BookHandler) GetBooksHandler(ctx *fiber.Ctx) error {
	...
	logger.Sugar().Infof("사용자의 책 목록을 성공적으로 조회했습니다. / 사용자ID: %s", userID.String())
...
func (h *BookHandler) GetBooksByUserNameHandler(ctx *fiber.Ctx) error {
	...
	if len(name) == 0 {
		logger.Sugar().Error("사용자 이름이 입력되지 않았습니다.")
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}
	...
	if err != nil {
		if ent.IsNotFound(err) || errors.Is(err, domain.ErrNotFound) {
			logger.Sugar().Errorf("등록된 책을 찾을 수 없습니다: %v", err)
			return ctx.Status(fiber.StatusNotFound).JSON(ErrorHandler(domain.ErrNotFound))
		}
		logger.Sugar().Errorf("책의 목록을 가져오는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
...
func (h *BookHandler) BookDeleteHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		...
		logger.Sugar().Errorf("책을 삭제하는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}

	logger.Sugar().Infof("책이 성공적으로 삭제되었습니다 / 책ID: %s, 사용자ID: %s", id, userID.String())
...
func (h *BookHandler) SearchBookIsbnHandler(ctx *fiber.Ctx) error {
	...
	logger.Sugar().Infof("검색하는 책 ISBN: %s", req.BookISBN)
	...
	if err != nil {
		logger.Sugar().Errorf("책 검색 API 요청 생성 실패: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
	...
	if err != nil {
		logger.Sugar().Errorf("책 검색 API 요청 실패: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
	...
	if err != nil {
		logger.Sugar().Errorf("책 검색 API 응답 본문 읽기 실패: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
	...
	if err != nil {
		logger.Sugar().Errorf("책 검색 API 응답 본문을 언마샬하는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}

	logger.Sugar().Infof("네이버 책 검색 API 요청이 성공적으로 완료되었습니다. / 사용자ID: %s", userID.String())
	logger.Sugar().Infof("네이버 책 검색 API 응답: %v", res)
...
func (h *BookHandler) SaveBookReviewHandler(ctx *fiber.Ctx) error {
	...
	// 필수 필드 검증
	if req.BookISBN == "" {
		logger.Sugar().Error("책 ISBN이 입력되지 않았습니다.")
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}
	if req.Rating < 1 || req.Rating > 5 {
		logger.Sugar().Error("별점은 1~5 사이의 값이어야 합니다.")
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}
	if req.Content == "" {
		logger.Sugar().Error("리뷰 내용이 입력되지 않았습니다.")
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}
	...
	if err != nil {
		...
		if err != nil {
			logger.Sugar().Errorf("해당 ISBN으로 등록된 책을 찾을 수 없습니다: %v", err)
			return ctx.Status(fiber.StatusNotFound).JSON(ErrorHandler(domain.ErrNotFound))
		}
	}
	...
	if err = h.bookUseCase.CreateReview(&domain.ReviewBook{
		...
		logger.Sugar().Errorf("책 리뷰 생성 중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}

	logger.Sugar().Infof("책 리뷰가 성공적으로 작성되었습니다 / 리뷰ID: %s, ISBN: %s", reviewID.String(), req.BookISBN)
...
func (h *BookHandler) GetBookReviewByUserIDHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.Sugar().Errorf("책 리뷰 목록을 가져오는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
...
func (h *BookHandler) GetPublicReviewsByBookIDHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.Sugar().Errorf("공개 리뷰 목록을 가져오는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
...
func (h *BookHandler) DeleteBookReviewHandler(ctx *fiber.Ctx) error {
	...
	if len(reviewID) == 0 {
		logger.Sugar().Error("리뷰 ID가 입력되지 않았습니다.")
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorHandler(domain.ErrInvalidInput))
	}
	...
	if err != nil {
		logger.Sugar().Errorf("리뷰 삭제 중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}

	logger.Sugar().Infof("리뷰가 성공적으로 삭제되었습니다 / 리뷰ID: %s", reviewID)
...
func (h *BookHandler) AddBookmarkHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.Sugar().Errorf("책 북마크 추가 중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
...
func (h *BookHandler) GetBookmarksByUserIDHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.Sugar().Errorf("책 북마크 목록을 가져오는 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
...
func (h *BookHandler) DeleteBookmarkHandler(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.Sugar().Errorf("서적의 북마크 제거하던 도중 오류가 발생했습니다: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(ErrorHandler(err))
	}
