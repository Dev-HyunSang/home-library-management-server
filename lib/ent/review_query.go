...
func (rq *ReviewQuery) QueryOwner() *UserQuery {
...
func (rq *ReviewQuery) QueryBook() *BookQuery {
...
func (rq *ReviewQuery) WithOwner(opts ...func(*UserQuery)) *ReviewQuery {
	query := (&UserClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) WithBook(opts ...func(*BookQuery)) *ReviewQuery {
	query := (&BookClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if rq.withOwner != nil || rq.withBook != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withOwner; query != nil {
		if err := rq.loadOwner(ctx, query, nodes, nil,
			func(n *Review, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withBook; query != nil {
		if err := rq.loadBook(ctx, query, nodes, nil,
...
func (rq *ReviewQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Review, init func(*Review), assign func(*Review, *User)) error {
...
func (rq *ReviewQuery) loadBook(ctx context.Context, query *BookQuery, nodes []*Review, init func(*Review), assign func(*Review, *Book)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Review)
	for i := range nodes {
